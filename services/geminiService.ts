...
export const startAnalysisStream = async (mode: AppMode, data: UserData, photoBase64?: string) => {
  try {
    ...
    const systemInstruction = `
      ...
      【分析指南】
      1. 保持客觀、專業、具備同理心。
      2. 針對「體質分析」，需綜合心理、情緒、飲食與主訴，尋找關鍵的療劑圖景 (Remedy Picture)。
      3. 針對「急救分析」，需快速識別病症特點與誘因。
      4. 必須包含 SRP (Strange, Rare and Peculiar) 症狀的考量。
    ...
    throw new Error(err.message || "AI 流式分析初始化失敗");
  }
};
